...
poc_url="%s"
'''
    content = template%(bug_id, kernel_map[kernel], k_id, syz_id, config_url, log_url, report_url,
                syz_url, poc_url)
    print(content)
    os.mkdir(ID)
...
def get_bug_info(table, repro_flag):
    ID = 0
    ...
    for tr in trs:
        ...
        if repro_flag and (not poc_url) and (not syz_url):
            continue
        ID += 1
        write_conf(kernel_repro, commit_id, syz_id, config_url, log_url, report_url,
